...
fun PartClock(
    ...
    Canvas(
        ...
        val hourIndicatorWidth = 3.dp.toPx()
        val minuteIndicatorWidth = 2.dp.toPx()
        val indicatorOffset = borderWidth + frameWidth / 2 + 2.dp.toPx()
        val minuteHandLength = radius - frameWidth / 2 - outlineWidth - 2.dp.toPx()
        val hourHandLength = minuteHandLength - hourIndicatorLength * 2 / 3
        val shadowDepth = borderWidth / 2

        drawClockFace(radius)
        drawMinuteIndicators(minuteIndicatorColor, minuteIndicatorLength, minuteIndicatorWidth, indicatorOffset)
        drawHourIndicators(hourIndicatorColor, hourIndicatorLength, hourIndicatorWidth, indicatorOffset)
